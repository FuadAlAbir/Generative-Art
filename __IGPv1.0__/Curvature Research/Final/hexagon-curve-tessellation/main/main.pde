...
float len = 40;                 // Side Length of Hexagons
float h = len * sqrt(3) / 2;    // Equilateral Triangle Height
float r = 8;                    // Curve Influencial Ellipse Radius
...
void draw()
{
  ...
  */
  ...
  for(float j = -h; j <= height + 2*h; j += 2*h)
  {
    counterJ++;
    counterI = 0;
    ...
    if(counterJ % 2 == 0)
    {
      for(float i = 0; i <= width + 2*len; i += len*2)
      {
        counterI++;
        {
          ...
          stroke(21, 80);
          strokeWeight(3);
          line(x + i, y + j, x - len/2 + i, y - h + j);                //1
          line(x - len/2 + i, y - h + j, x + i, y - 2*h + j);          //2
          line(x + i, y - 2*h + j, x + len + i, y - 2*h + j);          //3
          line(x + len + i, y - 2*h + j, x + 3*len/2 + i, y - h + j);  //4
          line(x + 3*len/2 + i, y - h + j, x + len + i, y + j);        //5
          line(x + len + i, y + j, x + i, y + j);                      //6
          */
          ...
          strokeWeight(2);
          noFill();
          curve(x - len/2 + c*cos(angle150) + i, y - h - c*sin(angle150) + j, x - len/2 + i, y - h + j, x + i, y + j, x + i, y + c + j);
          curve(x + i, y - 2*h - c + j, x + i, y - 2*h + j, x - len/2 + i, y - h + j, x - len/2 + c*cos(angle150) + i, y - h + c*sin(angle150) + j);
          curve(x + c*cos(angle150) + i, y - 2*h - c*sin(angle150) + j, x + i, y - 2*h + j, x + len + i, y - 2*h + j, x + len + c*cos(angle30) + i, y - 2*h - c*sin(angle30) + j);
          curve(x + len + i, y - 2*h - c + j, x + len + i, y - 2*h + j, x + 3*len/2 + i, y - h + j, x + 3*len/2 + c*cos(angle30) + i, y - h + c*sin(angle30) + j);
          curve(x + len + i, y + c + j, x + len + i, y + j, x + 3*len/2 + i, y - h + j, x + 3*len/2 + c*cos(angle30) + i, y - h - c*sin(angle30) + j);
          curve(x - c*cos(angle30) + i, y + c*sin(angle30) + j, x + i, y + j, x + len + i, y + j, x + len + c*cos(angle30) + i, y + c*sin(angle30) + j);
          ...
          strokeWeight(2);
          noFill();
          curve(x + c*cos(angle30) + i, y + c*sin(angle30) + j, x + i, y + j, x - len/2 + i, y - h + j, x - len/2 + i, y - h - c + j);
          curve(x - len/2 + i, y - h + c + j, x - len/2 + i, y - h + j, x + i, y - 2*h + j, x + c*cos(angle30) + i, y - 2*h - c*sin(angle30) + j);
          curve(x + c*cos(angle150) + i, y - 2*h + c*sin(angle150) + j, x + i, y - 2*h + j, x + len + i, y - 2*h + j, x + len + c*cos(angle30) + i, y - 2*h + c*sin(angle30) + j);
          curve(x + len + c*cos(angle150) + i, y - 2*h - c*sin(angle150) + j, x + len + i, y - 2*h + j, x + 3*len/2 + i, y - h + j, x + 3*len/2 + i, y - h + c + j);
          curve(x + 3*len/2 + i, y - h - c + j, x + 3*len/2 + i, y - h + j, x + len + i, y + j, x + len + c*cos(angle150) + i, y + c*sin(angle150) + j);
          curve(x - c*cos(angle30) + i, y - c*sin(angle30) + j, x + i, y + j, x + len + i, y + j, x + len + c*cos(angle30) + i, y - c*sin(angle30) + j);
          */
        }
      }
    ...
    {
      counterI = 1;
      for(float i = -len; i <= width; i += len*2)
      {
        counterI++;
        {
          if(counterI % 3 == 0) continue;
